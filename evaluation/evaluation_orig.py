...
class SymmetryPlaneEvaluation:
    ...
    def __repr__(self):
        return np.array2string(self.normal, precision=2, separator=',')+','+ np.array2string(self.point, precision=2, separator=',')+','+str(self.confidence)
...
def computeMetricsPerModel(inputRes, value, opt):
    ...
    with open(inputRes) as f:
        ...
        for i in range(num_symmetries):
            ...
            symmetry_list.append(SymmetryPlaneEvaluation(point=np.array(L[3:6]), normal=np.array(L[0:3]),confidence=L[6]))
