...
class Voxel:
    def __init__(self,
        ...
        self.original_points = pcd
        self.original_symmetries = [
            SymmetryPlane(
                point=sym_planes[idx, 3::].numpy(),
                normal=sym_planes[idx, 0:3].numpy()
            )
            for idx in range(sym_planes.shape[0])
        ]
        self.points, self.min, self.norm = normalize_pcd(self.original_points)
    ...
    def visualize_pcd(self):
        if self.env != "local":
            ...
            ps.register_point_cloud("Original pointcloud", self.original_points.detach().cpu().numpy())
            ps.register_point_cloud("Normalized pointcloud", self.points.detach().cpu().numpy())
            for idx, sym_plane in enumerate(self.original_symmetries):
